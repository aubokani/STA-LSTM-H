...
#SBATCH -J STALSTMH_loao                  # Job name
#SBATCH --array=1-18                      # One task per held-out animal
#SBATCH -c 8                              # 8 CPU cores per task
#SBATCH --mem=64G                         # 64 GB RAM (full cohort in memory)
#SBATCH -p gpucomputeq                    # GPU partition
#SBATCH --gres=gpu:1                      # 1 GPU per task
#SBATCH -t 12:00:00                       # 12-hour self-imposed cap (LOAO is ~3-4× per-animal)

#SBATCH -o logs/loao-%a.out
#SBATCH -e logs/loao-%a.err
